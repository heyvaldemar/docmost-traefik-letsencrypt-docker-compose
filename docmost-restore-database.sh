#!/usr/bin/env bash
# docmost-restore-database.sh [backup-file-name]
#
# Replaces the Docmost database with one of the dumps the backups service wrote.
#
#   ./docmost-restore-database.sh               list and ask
#   ./docmost-restore-database.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than docmost.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-docmost}"
APP_SERVICE="docmost"
...
DB_NAME="$(env_of DOCMOST_DB_NAME)"; DB_USER="$(env_of DOCMOST_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Docmost." >&2
  exit 1
fi
