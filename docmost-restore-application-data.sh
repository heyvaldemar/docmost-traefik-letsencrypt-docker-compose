...
BACKUP_PATH="/srv/docmost-application-data/backups/"
RESTORE_PATH="/app/data/storage/"
BACKUP_PREFIX="docmost-application-data"

echo "--> All available application data backups:"

for entry in $(docker container exec -it "$DOCMOST_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec -it "$DOCMOST_BACKUPS_CONTAINER" sh -c "rm -rf ${RESTORE_PATH}* && tar -zxpf ${BACKUP_PATH}${SELECTED_APPLICATION_BACKUP} -C /"
